...
12. ZIP-отчёт DoQA
Ответьте на сообщение разработчика:
/zip 671

671 — это ID в названии прогона DoQA. Бот найдёт самый свежий подходящий прогон и отправит ZIP ответом разработчику.

При временной ошибке бот повторит запрос автоматически. Если придёт сообщение об ошибке, ответьте на него одним словом:
повтори

После перезапуска бот продолжает незавершённые отчёты.
Отправленный ZIP добавляется в историю задачи. Ответ разработчика на ZIP относится к той же задаче и может завершить её.
